...
/* Prix de reference (assets/products.json) :
   ...
   spa-de-nage 9500 (sur commande, 50%) | spa-convivial 7500 (sur commande, 50%)
...
test("produit sur commande : encaisse 50% d'acompte", () => {
  ...
  assert.equal(o.subtotal, 7500);
  assert.equal(o.dueNow, 3750);
...
test("les options s'ajoutent au prix de base et suivent le taux d'acompte", () => {
  ...
  assert.equal(o.subtotal, 9500 + 1290 + 490); // 11280
  assert.equal(o.dueNow, 5640);
  assert.equal(o.amountInCents, 564000);
});

test("panier mixte : 100% sur le stock, 50% sur le sur-commande", () => {
  const o = computeOrder([
    { id: "nexus", qty: 1 },        // 4695 du
    { id: "spa-de-nage", qty: 1 },  // 9500 -> 4750 du
  ]);
  assert.equal(o.subtotal, 14195);
  assert.equal(o.dueNow, 9445);
...
test("le montant Stripe est un entier de centimes", () => {
  const o = computeOrder([{ id: "spa-de-nage", qty: 1 }]); // 9500 * 0.5 = 4750,00 €
